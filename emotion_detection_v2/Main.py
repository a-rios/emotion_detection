...
def main(args):
    ...
    model = Model.EmotionPrediction(args, logger)
    tokenizer =  model.get_tokenizer()

    #config = AutoConfig.from_pretrained(args.from_pretrained)
    #tokenizer = AutoTokenizer.from_pretrained(args.from_pretrained, cache_dir=args.cache_dir)
    #model = AutoModel.from_pretrained(args.from_pretrained, config=config, cache_dir=args.cache_dir)

    train_set = Data.EmotionDataset(in_file=args.train,
                                  tokenizer=tokenizer,
    ...
    dev_set = Data.EmotionDataset(in_file=args.dev,
                                  tokenizer=tokenizer,
                                  file_format=args.file_format,
                                  label_name=args.label_name,
                                  utterance_name=args.utterance_name,
                                  ...
                                  prediction_only=False)
    if args.test:
        test_set = Data.EmotionDataset(in_file=args.test,
                                    tokenizer=tokenizer,
                                    file_format=args.file_format,
                                    label_name=args.label_name,
                                    utterance_name=args.utterance_name,
